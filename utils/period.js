class GetPeriod {
  ...
  getPrevMonthStartDate() {
    let y = this.nowYear
    let m = this.nowMonth - 1

    if (m === 0) {
      y -= 1
      m = 12
    }
    return this.formatDate(new Date(y, m, 1));
  }
  // 获取上个月结束日期
  getPrevMonthEndDate() {
    let y = this.nowYear
    ...
    return this.formatDate(new Date(y, m, this.getMonthDays(m)));
  }

  // 获取最近三个月日期
  getLastThreeMonthStartDate() {
    const currentDate = new Date(this.now.getTime() - 90 * 24 * 60 * 60 * 1000)
    return this.formatDate(currentDate)
  }

  // 获取最近半年前的日期
  getLastSixMonthStartDate() {
    const currentDate = new Date(this.now.getTime() - 180 * 24 * 60 * 60 * 1000)
    return this.formatDate(currentDate)
  }
